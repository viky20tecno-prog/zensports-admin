...
import { formatCOP, formatDate, AFILIADO_TIER_PRICE } from '@/lib/utils';
import type { Afiliado, AfiliadoBillingRecord } from '@/types/afiliado';
...
interface Props {
  afiliado: Afiliado;
  initialRecords: AfiliadoBillingRecord[];
}

export function AfiliadoBillingTab({ afiliado, initialRecords }: Props) {
  const [records, setRecords] = useState<AfiliadoBillingRecord[]>(initialRecords);
  const [showForm, setShowForm] = useState(false);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');

  const tierPrice = AFILIADO_TIER_PRICE[afiliado.tier] ?? 0;
  ...
  const [form, setForm] = useState({
    monto: tierPrice > 0 ? String(tierPrice) : '',
  ...
  async function handleSubmit() {
    ...
    setSaving(true);
    const res = await fetch(`/api/afiliados/${afiliado.id}/billing`, {
      method: 'POST',
    ...
    if (res.ok) {
      ...
      setShowForm(false);
      setForm({ monto: tierPrice > 0 ? String(tierPrice) : '', periodo: currentPeriodo, metodo: 'transferencia', referencia: '', notas: '' });
    } else {
  ...
  const [boldResult, setBoldResult] = useState<AfiliadoBillingRecord | null>(null);
  ...
  async function handleGenerarLinkBold() {
    ...
    const res = await fetch(`/api/afiliados/${afiliado.id}/billing/bold-link`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  function startEdit(r: AfiliadoBillingRecord) {
    setEditingId(r.id);
    setEditError('');
    setEditForm({
  ...
  async function saveEdit(id: string) {
    ...
    setEditSaving(true);
    setEditError('');
    const res = await fetch(`/api/afiliados/${afiliado.id}/billing/${id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function handleDelete(r: AfiliadoBillingRecord) {
    if (!window.confirm(`¿Borrar el pago de ${formatCOP(r.monto)} (${formatPeriodo(r.periodo)})? Esta acción no se puede deshacer.`)) return;
    setDeletingId(r.id);
    const res = await fetch(`/api/afiliados/${afiliado.id}/billing/${r.id}`, { method: 'DELETE' });
    setDeletingId(null);
    if (res.ok) {
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="rounded-xl border border-white/8 bg-white/2 p-4">
          <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Tier actual</p>
          <p className="text-lg font-bold text-white capitalize">{afiliado.tier}</p>
          {tierPrice > 0 && <p className="text-xs text-gray-600 mt-0.5">{formatCOP(tierPrice)}/mes</p>}
        </div>
        <div className="rounded-xl border border-white/8 bg-white/2 p-4">
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-medium text-gray-400 flex items-center gap-2">
          <TrendingUp className="w-4 h-4" /> Historial de membresía
        </h3>
      ...
      {showBoldForm && (
        <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs text-gray-500 mb-1">Monto (tier {afiliado.tier}: {formatCOP(tierPrice)})</label>
              <input
                type="number"
                value={boldMontoOverride}
                onChange={e => setBoldMontoOverride(e.target.value)}
                placeholder={String(tierPrice)}
      ...
      {showForm && (
        <div className="rounded-xl border border-indigo-500/20 bg-indigo-500/5 p-4 space-y-3">
          <p className="text-xs font-medium text-indigo-400 uppercase tracking-wider">Nuevo pago de membresía</p>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs text-gray-500 mb-1">Período *</label>
              <input
      ...
      {records.length === 0 ? (
        <div className="rounded-xl border border-white/8 py-14 text-center text-gray-600 text-sm">
          Sin pagos de membresía registrados
        </div>
      ) : (
